...
uint32_t br_sphincs_p_third_party_sign(const br_sphincs_p_private_key *sk,
    ...
    case sha256_128f_robust:
        rc = OQS_SIG_sphincs_sha256_128f_robust_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_128f_simple:
        rc = OQS_SIG_sphincs_sha256_128f_simple_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_128s_robust:
        rc = OQS_SIG_sphincs_sha256_128s_robust_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_128s_simple:
        rc = OQS_SIG_sphincs_sha256_128s_simple_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_192f_robust:
        rc = OQS_SIG_sphincs_sha256_192f_robust_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_192f_simple:
        rc = OQS_SIG_sphincs_sha256_192f_simple_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_192s_robust:
        rc = OQS_SIG_sphincs_sha256_192s_robust_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_192s_simple:
        rc = OQS_SIG_sphincs_sha256_192s_simple_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_256f_robust:
        rc = OQS_SIG_sphincs_sha256_256f_robust_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_256f_simple:
        rc = OQS_SIG_sphincs_sha256_256f_simple_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_256s_robust:
        rc = OQS_SIG_sphincs_sha256_256s_robust_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
    case sha256_256s_simple:
        rc = OQS_SIG_sphincs_sha256_256s_simple_sign(sig, &sig_len, msg, msg_len, sk->k);
        if (rc == OQS_SUCCESS) {
            return sig_len;
        }
        break;
